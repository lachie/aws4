...
var s3region map[string]string = map[string]string{
...
func parseHost(host string) (string, string, error) {
  // s3 is inconsistent
  if s3region := s3region[host]; s3region != "" {
    return "s3", s3region, nil
  }

  parts := strings.Split(host, ".")
  if len(parts) < 4 {
    return "", "", fmt.Errorf("Invalid AWS Endpoint: %s", host)
  }
...
func Sign(keys *Keys, r *http.Request) error {
  name, region, err := parseHost(r.Host)
  if err != nil {
    return err
  }
...
func (s *Service) writeRequest(w io.Writer, r *http.Request) {
  // TODO only write this if not set
